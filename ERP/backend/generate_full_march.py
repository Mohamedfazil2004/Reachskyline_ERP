...
def _assign_to_best_writer(writer_mins, mins_needed):
    ...
    for wid, info in writer_mins.items():
        if info['remaining'] >= mins_needed:
            emp = info['employee']
            efficiency = float(getattr(emp, 'efficiency_score', 5) or 5)
            capacity_ratio = float(info['remaining'] / info['rec'].available_minutes)
            score = (efficiency * 0.7) + (capacity_ratio * 3.0) 
            if score > best_score:
                best = info
                best_score = score
    return best
...
def run_generation(target_date):
    ...
    for item in deliv_with_remaining:
        ...
        # Schedule tasks based on remaining days in month
...
if __name__ == '__main__':
    with app.app_context():
        print("Starting full generation for March 2026 to reach 202 tasks...")
        total_created = 0
        for day in range(1, 32):
            d = date(2026, 3, day)
            count = run_generation(d)
            total_created += count
            if count > 0:
                print(f"Generated {count} tasks for {d}")
        
        final_count = AutomationTask.query.filter_by(month=3, year=2026).count()
        print(f"DONE! Total tasks in DB for March: {final_count}")
