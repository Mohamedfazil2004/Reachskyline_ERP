...
import {
    FaUserCheck, FaCalendarAlt, FaChartLine, FaListAlt, FaArrowLeft, FaChartBar, FaTasks, FaCheckCircle, FaRegListAlt
} from 'react-icons/fa';
...
function EfficiencyLineChart({ data, color, onPointClick }) {
    ...
    const gradId = `eff-grad-editor-${color.replace('#', '')}`;
...
const EditorDashboard = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const effRes = await axios.get(`${API}/efficiency-history?month=3&year=2026`, { headers });
            setEffHistory(effRes.data);
        } catch (err) {
    ...
    const todayStats = effHistory.history.find(h => h.date === todayStr) || { assigned: 0, available: 480, worked: 0, task_count: 0 };

    return (
        <div className="animate-fade-in" style={{ paddingBottom: '40px' }}>

            {/* Welcome Banner - Top Full Width like Content Writer */}
            <div className="card" style={{ display: 'flex', flexDirection: window.innerWidth < 768 ? 'column' : 'row', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(135deg, #6366f1 0%, #4f46e5 100%)', color: '#fff', borderRadius: '25px', padding: '30px', boxShadow: '0 10px 25px rgba(99, 102, 241, 0.3)', marginBottom: '30px', gap: '20px' }}>
                <div>
                    <h2 style={{ margin: 0, fontSize: '1.8rem', fontWeight: 800 }}>
                        Welcome, {(user?.name || 'Editor').split(' ')[0]}! 🎬
                    </h2>
                    <p style={{ margin: '8px 0 0', opacity: 0.9, fontSize: '1.1rem' }}>
                        Ready to edit some amazing content today?
                    </p>
                </div>
            ...
            {/* Efficiency Grid - Left Detail & Right Monthly History */}
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px, 1fr) 2fr', gap: '20px' }}>

                {/* Left: Avg Efficiency Chart Card */}
                ...
                {/* Right: Monthly Efficiency Record */}
